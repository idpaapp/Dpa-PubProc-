...
public class PersianCalendar {
    ...
    private String[] mGMonthNamesPer = {
    		"ژانویه",
			"فوریه",
			"مارس",
			"آپریل",
    ...
    private void GregorianDateFromInt(int DateValue) {    	
        ...
        if (gD <= 0)
        	switch (gM) {
			case 1 : {gD = 31; break;}
			case 2 : { if (IsLeapYearGre(gY)) gD = 29; else gD = 28; break;}
			case 3 : {gD = 31; break;}
			case 4 : {gD = 30; break;}
			case 5 : {gD = 31; break;}
			case 6 : {gD = 30; break;}
    ...
    public String PersianSimpleDate(int aDate) {
    	PersianDateFromInt(aDate);
    	return String.valueOf(this.jY) + "/" + ifOneChar(this.jM)+"/"+ ifOneChar(this.jD);
    }
    
    public String PersianSimpleDate(int year, int month, int day) {
    	return String.valueOf(year) + "/" + ifOneChar(month)+"/"+ ifOneChar(day);
    }
    
    public String PersianFullDate(int aDate) {
    	PersianDateFromInt(aDate);
    	return String.valueOf(this.jD) + " " +GetMonthName(jM)+ " "+ this.jY;
    }
    
    public String PersianFullDateDesc(int aDate) {
    	PersianDateFromInt(aDate);
    	return getDayName(DayOfWeek(this.jY, this.jM, this.jD)) +" "+ String.valueOf(this.jD) + " " +GetMonthName(jM)+ " "+ this.jY;
    }
    
    public String PersianFullDateDesc(int year, int month, int day) {
    	return getDayName(DayOfWeek(year, month, day)) +" "+ String.valueOf(day) + " " +GetMonthName(month)+ " "+ year;
    }
    
    public String PersianToGregorianFullDate(int year, int month, int day, boolean PersianMonthName) {
    	if(year == 0) return "";
    	int jInt = PersianDateToInt(year, month, day);  
    	jInt += 226899;
    	GregorianDateFromInt(jInt);
    	...
    	if (PersianMonthName)
    		return String.valueOf(gD) + " " + GetGMonthNamePer(gM)+ " " + String.valueOf(gY);
    	else
    		return String.valueOf(gD) + " " + GetGMonthName(gM)+ " " + String.valueOf(gY);
    } 
    
    public String AddDay(int year, int month, int day, int Adddays) {
    	if (year == 0) return "";
        ...
        return PersianSimpleDate(ActDay);
    }
    
    public String GetDayByDate(String ActiveDate)
    {
    	return ActiveDate.substring(8);
    }

    public String GetMonthByDate(String ActiveDate)
    {
    	return ActiveDate.substring(5,7);
    }
    
    public String GetYearByDate(String ActiveDate)
    {
    ...
    public boolean IsLeapYearGre(int y)
    {
    	return (y % 4 == 0);
    }
	...
	public String getWeekDayNamebyDay(int year, int month, int day)
	{
		int mday = DayOfWeek(year, month, day);
		return getDayName(mday);
	}
    ...
    public String GetGMonthNamePer(int monthcode)
    {
    	if (monthcode == 0)
    		return "";
    	return mGMonthNamesPer[monthcode - 1];
    }
